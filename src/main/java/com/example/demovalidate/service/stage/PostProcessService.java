...
@Slf4j
@Service
public class PostProcessService implements BusinessProcessBase, Ordered {
    @Autowired
    private List<PostNodeProcessBase> nodeProcessList;
    /*@Autowired
    ...
    public Object process(ProcessEvent event) {
        if(isCurrentStage(event)){
            log.info("不是当前阶段，不可执行");
            return null;
        }
        //执行节点操作
    ...
    public Object process(ProcessBusinessDto dto) {
        /*String[] beanNames = ctx.getBeanNamesForType(nodeProcessList.get(0).getClass());
